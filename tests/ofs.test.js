...
import { createTestUser, getAuthToken, createTestColumn, createTestOF, } from './helpers.js';
import './setup.js';
const app = createApp();
describe('OFs Routes', () => {
    let authToken;
    ...
    describe('GET /api/ofs', () => {
        ...
        it('should filter OFs by columnId', async () => {
            ...
            const of2 = await createTestOF(column2._id.toString(), 'OF in Column 2', undefined, 0);
            const response = await request(app)
    ...
    describe('GET /api/ofs/:id', () => {
        it('should return a single OF by id', async () => {
            ...
            expect(response.body.data._id).toBe(of._id.toString());
        });
    ...
    describe('POST /api/ofs', () => {
        it('should create a new OF', async () => {
            ...
            expect(response.body.data).toHaveProperty('columnId', columnId.toString());
    ...
    describe('PATCH /api/ofs/:id/move', () => {
        it('should move OF to a different column', async () => {
            ...
            const response = await request(app)
                ...
                .set('Authorization', `Bearer ${authToken}`)
                .send({
                columnId: column2._id.toString(),
            })
                .expect(200);
            expect(response.body).toHaveProperty('message', 'OF moved successfully');
            expect(response.body.data.columnId).toBe(column2._id.toString());
        });
...
//# sourceMappingURL=ofs.test.js.map