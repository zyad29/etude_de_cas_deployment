 ...
 * ```
 */
export function getAuthToken(user, secret = process.env.JWT_SECRET || 'test-secret-key-at-least-32-characters') {
 ...
 * ```
 */
export async function createTestColumn(name = 'Test Column', position = 0) {
 ...
 * ```
 */
export async function createTestOF(columnId, title = 'Test OF', description, position = 0) {
    const of = new OF({
        title,
        description,
        columnId,
